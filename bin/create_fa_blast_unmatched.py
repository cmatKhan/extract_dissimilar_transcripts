# -*- coding: utf-8 -*-
"""
name: create_fa_blast_unmatched.py
purpose: extract a .fa of sequences not matched in blast results
input: .fa used to create blast results in question and the unique tx names of the transcripts matched to some sequence by blast
...
# purpose: create dataframe from .fasta
    with open(path_to_fa) as fasta_file:  # Will close handle cleanly
        ...
        for seq_record in SeqIO.parse(fasta_file, 'fasta'):  # (generator)
            ...
            description.append(str(seq_record.description).split(' ', 1)[1])
            seq.append(str(seq_record.seq))

    my_dict = {'id':identifiers, 'desc': description, 'seq': seq}
...
[fa.write('>'+name+'\n'+seq+'\n') for name,seq in zip(unmatched_records['name'], unmatched_records['seq'])]
fa.close()
